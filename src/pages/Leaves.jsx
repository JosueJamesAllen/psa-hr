...
export default function Leaves() {
  // loaded once
  const [bootstrap, setBootstrap] = useState(null); // { employees, seats, unavailability }
  // per-applicant
  const [applicantId, setApplicantId] = useState(null);
  const [typeData, setTypeData] = useState(null); // { types, balances }
  ...
  useEffect(() => {
    let ok = true;
    Promise.all([listEmployees(), getSignatorySeats(), getSignatoryUnavailability()]).then(
      ([emps, seats, unavailability]) => {
        if (!ok) return;
        setBootstrap({ employees: emps, seats, unavailability });
        setApplicantId(emps[0]?.id ?? null);
      },
    );
    return () => { ok = false; };
  }, []);

  // reload leave types + balances whenever the applicant changes
  useEffect(() => {
    if (!bootstrap || !applicantId) return;
    const emp = bootstrap.employees.find((e) => e.id === applicantId);
    let ok = true;
    Promise.all([listLeaveTypes(emp.empClass), getBalances(applicantId)]).then(([types, balances]) => {
      if (!ok) return;
      setTypeData({ types, balances });
      setTypeCode(types[0]?.code ?? null); // keep selection valid for this class
    });
    return () => { ok = false; };
  }, [bootstrap, applicantId]);

  const applicant = bootstrap?.employees.find((e) => e.id === applicantId) ?? null;
  const leaveType = typeData?.types.find((t) => t.code === typeCode) ?? typeData?.types[0] ?? null;

  const datesValid = start && end && start <= end;
  const workingDays = datesValid ? computeWorkingDays(start, end) : 0;

  const routing = useMemo(() => {
    if (!leaveType || !bootstrap) return null;
    return resolveRouting({ applicantSeat: applicant?.seat ?? null, chain: leaveType.routing, seats: bootstrap.seats });
  }, [leaveType, applicant, bootstrap]);

  const credit = useMemo(() => {
    if (!leaveType || !datesValid || workingDays === 0) return null;
    if (leaveType.isPaid && leaveType.requiresCredits) {
      const balance = typeData?.balances?.[leaveType.category] ?? 0;
      return { kind: "credit", balance, after: balance - workingDays, enough: hasEnoughCredits(balance, workingDays) };
    }
    if (!leaveType.isPaid) {
      return { kind: "deduction", amount: coswDeduction(applicant, workingDays), half: payrollHalf(start) };
    }
    return { kind: "paid_no_credit" };
  }, [leaveType, typeData, applicant, workingDays, datesValid, start]);

  const filing = useMemo(() => {
    if (!leaveType || !routing || !bootstrap) return null;
    const isAvailable = (seatObj, dateStr) =>
      !bootstrap.unavailability.some((u) => u.seat === seatObj.seat && dateStr >= u.start && dateStr <= u.end);
    return checkFilingDate({
      filingDate,
      inclusiveStart: start,
      advanceMinDays: leaveType.advanceMinDays,
      advanceMaxDays: leaveType.advanceMaxDays,
      requiredSeats: routing.requiredForAvailability,
      isAvailable,
    });
  }, [filingDate, start, leaveType, routing, bootstrap]);
    ...
    <div className="mx-auto max-w-5xl p-6">
      <header className="mb-6">
        ...
        <p className="mt-1 text-sm text-slate-600 dark:text-slate-400">
          Fill in the details; the summary on the right updates as you go and flags anything that would block filing.
        </p>
      </header>
      ...
      <div className="grid gap-6 lg:grid-cols-[1fr_20rem]">
        <form onSubmit={handleSubmit} className="space-y-5 rounded-2xl border border-slate-200 bg-white p-6 shadow-sm dark:border-slate-800 dark:bg-slate-900/60">
          <div>
            ...
            <select id="applicant" value={applicantId} onChange={(e) => setApplicantId(e.target.value)} className={fieldClass}>
              {bootstrap.employees.map((emp) => (
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label htmlFor="start" className={labelClass}>First day</label>
              <input id="start" type="date" value={start} onChange={(e) => setStart(e.target.value)} className={fieldClass} />
            </div>
            <div>
              <label htmlFor="end" className={labelClass}>Last day</label>
              <input id="end" type="date" value={end} min={start} onChange={(e) => setEnd(e.target.value)} className={fieldClass} />
            </div>
          </div>
          ...
          <div>
            ...
            {leaveType.advanceMinDays != null ? (
              <p className="mt-1 text-xs text-slate-500 dark:text-slate-400">
                File {leaveType.advanceMinDays}\u2013{leaveType.advanceMaxDays} days before the leave, on a day all signatories can sign.
              </p>
            ) : (
          ...
          <div>
            <label htmlFor="purpose" className={labelClass}>Purpose / details</label>
            <textarea id="purpose" rows={3} value={purpose} onChange={(e) => setPurpose(e.target.value)} className={fieldClass} placeholder="Optional" />
          </div>
        ...
        <aside className="lg:sticky lg:top-6 lg:self-start">
          <div className="space-y-4 rounded-2xl border border-slate-200 bg-slate-50 p-5 dark:border-slate-800 dark:bg-slate-900/40">
            ...
            <div className="border-t border-slate-200 pt-4 dark:border-slate-800">
              ...
              {filing?.ok ? (
                <Note tone="ok">{prettyDate(filingDate)} works \u2014 all signatories available.</Note>
              ) : (
                <>
                  <Note tone="bad">{FILING_REASONS[filing?.reason] ?? "This date can't be used."}</Note>
                  {filing?.suggested && (
                    <button type="button" onClick={() => setFilingDate(filing.suggested)} className="mt-2 w-full rounded-lg border border-psa-blue px-3 py-1.5 text-sm font-medium text-psa-blue hover:bg-psa-blue/5">
                      Use {prettyDate(filing.suggested)}
                    </button>
                  )}
                </>
              )}
            ...
            {filed.map((a) => (
              <li key={a.id} className="flex flex-wrap items-center justify-between gap-2 bg-white px-4 py-3 text-sm dark:bg-slate-900/40">
                ...
                <span className="text-slate-500 dark:text-slate-400">
                  {prettyDate(a.start)}\u2013{prettyDate(a.end)} \u00B7 {a.workingDays} day{a.workingDays > 1 ? "s" : ""}
                </span>
...
function Row({ label, value, tone }) {
  const toneClass = tone === "ok" ? "text-green-600 dark:text-green-400" : tone === "bad" ? "text-psa-red" : tone === "warn" ? "text-amber-600 dark:text-amber-400" : "text-slate-800 dark:text-slate-200";
  return (
  ...
  return (
    <li className="flex items-center justify-between gap-3">
      ...
      <span className="text-right font-medium text-slate-800 dark:text-slate-200">
        {who}
        {external && <span className="ml-1 rounded bg-psa-gold/20 px-1.5 py-0.5 text-xs text-amber-700 dark:text-amber-300">external</span>}
      </span>
    </li>
  );
}
...
function Note({ tone, children }) {
  const map = {
    ok: "bg-green-50 text-green-700 dark:bg-green-950/40 dark:text-green-300",
    bad: "bg-red-50 text-psa-red dark:bg-red-950/40 dark:text-red-300",
